...
annotate_drug_results <- function(results_df,
  ...
  sort_by_score <- function(df, score_col, decreasing = FALSE) {
    if (is.null(df) || nrow(df) == 0) return(df)
    if (!score_col %in% names(df)) return(df)

    sc <- suppressWarnings(as.numeric(df[[score_col]]))
    ord <- order(sc, na.last = TRUE, decreasing = decreasing)
    df[ord, , drop = FALSE]
  }
  ...
  sig_cols <- c("sig_id", "pert_type", "pert_id", "pert_iname", "cmap_name")
  ...
  tech_view <- sort_by_score(tech_view, score_col, decreasing = FALSE)
